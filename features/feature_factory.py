"""
MODULE: features/feature_factory.py
PURPOSE: Main feature engineering orchestrator. Synchronizes DOM and trade data
         on a 1-minute time grid and computes microstructure features.
INPUT: Cleaned DOM snapshots DataFrame and inferred trades DataFrame.
OUTPUT: Combined Feature Matrix aggregated to 1-minute bars.

WHY IT IS NEEDED:
Microstructure signals occur at high frequency, but for machine learning models
and backtesting, we aggregate them into standard 1-minute intervals. This factory
aligns the order book and trade flow data onto a synchronized time grid, handles
gaps, and computes features incrementally.
...
class FeatureFactory:
    ...
    def build_feature_matrix(
        ...
        """
        Orchestrate the feature generation pipeline for Tick + DOM validated windows.

        Parameters
        ----------
        df_dom : pd.DataFrame
            Cleaned DOM snapshots
        df_tick : pd.DataFrame
            Cleaned Tick records
        df_trades : pd.DataFrame, optional
            Inferred trade records. If omitted, trades are inferred from DOM.
        tolerance_s : float, optional
            Event-time alignment tolerance in seconds.

        Returns
        -------
        pd.DataFrame
            Feature matrix (1-minute bars) for validated windows only.
        """
        if len(df_dom) == 0:
        ...
        # ------------------------------------------------------------------------
        # STEP 1: Detect data source type and optionally align Tick + DOM events
        # ------------------------------------------------------------------------
        # Auto-detect unified marketfeed: if df_tick already has DOM depth columns,
        # check that at least 5 bid/ask levels are present (depth 5 requirement).
        # For past data with separate DOM + Tick files (or tick with <5 levels), run alignment.
        ...
        # ------------------------------------------------------------------------
        # ------------------------------------------------------------------------
        # STEP 2: Resample DOM snapshots to 1-minute bars using DuckDB
        # ------------------------------------------------------------------------
        logger.info("Resampling DOM data to 1-minute bars (taking mean) using DuckDB...")
        import duckdb
        con = duckdb.connect()
        con.execute("SET TIME ZONE 'UTC'")

        # For unified feed, df_tick already has DOM columns — use it as DOM source
        ...
        # Recompute total bid/ask qty from ONLY the top available depth levels (max 5).
        # Separate DOM files have all 20 levels; unified tick may have fewer.
        ...
        # Compute Stage 3 market depth features on the resampled DOM bars.
        ...
        # Microstructure proxies derived from DOM depth changes
        dom_1m['bid_replenishment_rate'] = FeatureFactory._compute_bid_replenishment_rate(dom_1m)

        # Filter to alignment-validated windows (separate files) or keep all (unified feed)
        ...
        # ------------------------------------------------------------------------
        # STEP 3: Aggregate trade records to 1-minute bins
        # ------------------------------------------------------------------------
        ...
        if len(df_trades) == 0:
            ...
            for idx, row in df_trades.iterrows():
                qty = int(row['trade_qty'])
                qtys.append(qty)
                n = len(qtys)
                if n >= 3 and (n <= 50 and n % 5 == 0 or n > 50 and n % 50 == 0):
                    threshold = 10 * statistics.median(qtys)
                if threshold != float('inf') and qty > threshold:
                    is_large_list.append(1)
                else:
                    is_large_list.append(0)
            df_trades['is_large'] = is_large_list
            median_size = float(np.median(df_trades['trade_qty'].values))
        ...
        # Retain only windows that passed alignment validation.
        if valid_windows is not None:
    ...
    def _compute_imbalance(df: pd.DataFrame, levels: int) -> pd.Series:
        # Use whatever depth columns exist (up to the requested level count)
    ...
    def validate_feature_ranges(df: pd.DataFrame) -> bool:
        """
        Validate that calculated features lie within expected physical ranges.
        Logs warnings for any outliers.
        
        Parameters
        ----------
        df : pd.DataFrame
            Calculated features DataFrame
            
        Returns
        -------
        bool
            True if all checks pass, False if any outliers are found.
        """
        passed = True
